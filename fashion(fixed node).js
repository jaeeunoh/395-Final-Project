	...
	.await(function(error, node_list, edge_list, node_url) {
			...
			for (var i in nodes) {
				...
				if (nodes[i].id == url.Node) {
					...
					if (nodes[i].group == "Event") {
						...
						console.log("EVENT" + url.url_event);
					}
				}
		...
		d3.select("#autocomplete").on("click", function() {
			...
			if (selectedVal == "none") {
				...
				//Filter node and linked not searched and set their opacities to 0
				// var selected = node.filter(function(d) {
				// 	return d.group == "Clothing" && temp.indexOf(d.id) == -1;
				// });
				// selected.style("opacity", "0");
				console.log(selectedNodes);
				searchNode(selectedNodes);
				// var link = svg.selectAll(".link")
				// var selected_link = link.filter(function(d) {
				// 	return temp.indexOf(d.source.id) == -1;
				// })
				// selected_link
				// .style("opacity", 0);
		...
		d3.select("#reset").on("click", function() {
			...
			selectedNodes = [];
			reset();
			// svg_network.selectAll("circle")
			// .transition()
			// .duration(1000)
			// .style("opacity", 1);

			// svg_network.selectAll("path.link").transition()
			// .duration(1000)
			// .style("opacity", 0.3);
		})
